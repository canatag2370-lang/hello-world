...
1. Download the project files once (for example with **Code → Download ZIP** on
   ...
   run the app.
2. Make sure you have Python 3.8–3.11 installed. DeepFace (the library used for
   gender and age predictions) currently does **not** ship wheels for Python
   3.12+, so using 3.10 or 3.11 is strongly recommended. On Windows you can
   verify the version by opening **PowerShell** and running `python --version`
   or `py --version`.
...
5. Grant camera access if your system asks for permission. When the window
   ...
   If the yellow age/gender line does not appear, check the terminal once: the
   app prints a one-time warning if DeepFace or TensorFlow is missing so you
   know which dependency to install. Eğer `ModuleNotFoundError: No module`
   `named 'deepface'` hatasını görürseniz, script otomatik olarak paketi kurmayı
   deneyecek. Sorun devam ederse `python run_face_distance.py` komutunu tekrar
   çalıştırabilir veya `pip install deepface` komutuyla paketi manuel
   kurabilirsiniz. Python 3.12 kullanıyorsanız DeepFace henüz bu sürüm için
   yayınlanmadığından tahminler devre dışı kalır; böyle bir durumda Python 3.10
   veya 3.11 ile tekrar denemeniz gerekir.
